...
class ThumbsUpAni {
    ...
    createRender(i) {
        ...
        const getTranslateY = (diffTime) => {
            return image[i].height / 2 + (this.height - image[i].height / 2) * (1 - diffTime);
        };
    ...
    start() {
        for (let i = 0; i < this.imgsList.length; i++) {
            let render = this.createRender(i);
            let duration = getRandom(1500, 3000);
